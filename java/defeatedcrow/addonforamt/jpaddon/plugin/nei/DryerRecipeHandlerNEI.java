...
public class DryerRecipeHandlerNEI extends TemplateRecipeHandler {

	private List<IDryerRecipe> thisRecipes;

	private List<IDryerRecipe> recipeLoader() {
		if (RecipeManagerJP.dryerRecipe.getRecipes() != null && !RecipeManagerJP.dryerRecipe.getRecipes().isEmpty()) {
			this.thisRecipes = (List<IDryerRecipe>) RecipeManagerJP.dryerRecipe.getRecipes();
		}
		return thisRecipes;
	}
	...
	public String getOverlayIdentifier() {
		return "DCsDryingRack";
	}

	@Override
	public void loadTransferRects() {
		transferRects
				.add(new TemplateRecipeHandler.RecipeTransferRect(new Rectangle(135, 20, 13, 13), "DCsDryingRack"));
	}

	@Override
	public void loadCraftingRecipes(String outputId, Object... results) {
		if (outputId.equals("DCsDryingRack") && getClass() == DryerRecipeHandlerNEI.class) {
			List<IDryerRecipe> recipes = this.recipeLoader();
	...
	public String getRecipeName() {
		return StatCollector.translateToLocal("amtjp.nei.dryer_recipe");
	}
	...
	public void drawExtras(int recipe) {
		drawProgressBar(48, 20, 176, 0, 16, 12, 24, 3);

		// mode icon
		Minecraft mc = Minecraft.getMinecraft();
		ResourceLocation res = new ResourceLocation("amtjp:textures/gui/dryer_nei.png");
		mc.getTextureManager().bindTexture(res);
		mc.currentScreen.drawTexturedModalRect(135, 20, 192, 26, 13, 13);
